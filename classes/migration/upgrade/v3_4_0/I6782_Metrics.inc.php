...
class I6782_Metrics extends Migration
{
    ...
    public function up()
    {
        ...
        DB::statement("INSERT INTO metrics_context (load_id, context_id, date, metric) SELECT m.load_id, m.assoc_id, {$dayFormatSql}, m.metric FROM metrics m WHERE m.assoc_type = 512 AND m.metric_type = 'omp::counter'");
        // Migrate series metrics
        DB::statement("INSERT INTO metrics_series (load_id, context_id, series_id, date, metric) SELECT m.load_id, m.context_id, m.assoc_id, {$dayFormatSql}, m.metric FROM metrics m WHERE m.assoc_type = 530 AND m.metric_type = 'omp::counter'");
        ...
        // Insert into metrics_counter_submission_geo_daily table
        // metric_book_investigations
        // means: abstracts + galley and supp files that do not belong to a chapter
        $metricBookInvestigationsUpsertSql = "
                INSERT INTO metrics_counter_submission_geo_daily (load_id, context_id, submission_id, country, region, city, date, metric_book_investigations, metric_book_investigations_unique, metric_book_requests, metric_book_requests_unique, metric_chapter_investigations, metric_chapter_investigations_unique, metric_chapter_requests, metric_chapter_requests_unique, metric_title_investigations_unique, metric_title_requests_unique)
                SELECT * FROM (SELECT m.load_id, m.context_id, m.submission_id, COALESCE(m.country_id, ''), COALESCE(m.region, ''), COALESCE(m.city, ''), {$dayFormatSql} as mday, SUM(m.metric) as metric, 0 as metric_book_investigations_unique, 0 as metric_book_requests, 0 as metric_book_requests_unique, 0 as metric_chapter_investigations, 0 as metric_chapter_investigations_unique, 0 as metric_chapter_requests, 0 as metric_chapter_requests_unique, 0 as metric_title_investigations_unique, 0 as metric_title_requests_unique
                    FROM metrics m
                    LEFT JOIN submission_file_settings sfs ON (m.assoc_type IN (515, 531) AND m.assoc_id = sfs.submission_file_id AND sfs.setting_name = 'chapterId')
                    WHERE m.assoc_type IN (515, 531, 1048585) AND m.metric_type = 'omp::counter' AND sfs.setting_value IS NULL AND (m.country_id IS NOT NULL OR m.region IS NOT NULL OR m.city IS NOT NULL)
                    GROUP BY m.load_id, m.context_id, m.submission_id, m.country_id, m.region, m.city, mday) AS t
            ";
        if (substr(Config::getVar('database', 'driver'), 0, strlen('postgres')) === 'postgres') {
            $metricBookInvestigationsUpsertSql .= '
        ...
        DB::statement($metricBookInvestigationsUpsertSql);

        // metric_book_requests
        // means: galley files that do not belong to a chapter
        $metricBookRequestsUpsertSql = "
                INSERT INTO metrics_counter_submission_geo_daily (load_id, context_id, submission_id, country, region, city, date, metric_book_investigations, metric_book_investigations_unique, metric_book_requests, metric_book_requests_unique, metric_chapter_investigations, metric_chapter_investigations_unique, metric_chapter_requests, metric_chapter_requests_unique, metric_title_investigations_unique, metric_title_requests_unique)
                SELECT * FROM (SELECT m.load_id, m.context_id, m.submission_id, COALESCE(m.country_id, ''), COALESCE(m.region, ''), COALESCE(m.city, ''), {$dayFormatSql} as mday, 0 as metric_book_investigations, 0 as metric_book_investigations_unique, SUM(m.metric) as metric, 0 as metric_book_requests_unique, 0 as metric_chapter_investigations, 0 as metric_chapter_investigations_unique, 0 as metric_chapter_requests, 0 as metric_chapter_requests_unique, 0 as metric_title_investigations_unique, 0 as metric_title_requests_unique
                    FROM metrics m
                    LEFT JOIN submission_file_settings sfs ON (m.assoc_type = 515 AND m.assoc_id = sfs.submission_file_id AND sfs.setting_name = 'chapterId')
                    WHERE m.assoc_type = 515 AND m.metric_type = 'omp::counter' AND sfs.setting_value IS NULL AND (m.country_id IS NOT NULL OR m.region IS NOT NULL OR m.city IS NOT NULL)
                    GROUP BY m.load_id, m.context_id, m.submission_id, m.country_id, m.region, m.city, mday) AS t
            ";
        if (substr(Config::getVar('database', 'driver'), 0, strlen('postgres')) === 'postgres') {
            $metricBookRequestsUpsertSql .= '
        ...
        DB::statement($metricBookRequestsUpsertSql);

        // metric_chapter_investigations
        // means: galley and supp files that belong to a chapter
        // chapter landing page did not exist yet
        $metricChapterInvestigationsUpsertSql = "
                INSERT INTO metrics_counter_submission_geo_daily (load_id, context_id, submission_id, country, region, city, date, metric_book_investigations, metric_book_investigations_unique, metric_book_requests, metric_book_requests_unique, metric_chapter_investigations, metric_chapter_investigations_unique, metric_chapter_requests, metric_chapter_requests_unique, metric_title_investigations_unique, metric_title_requests_unique)
                SELECT * FROM (SELECT m.load_id, m.context_id, m.submission_id, COALESCE(m.country_id, ''), COALESCE(m.region, ''), COALESCE(m.city, ''), {$dayFormatSql} as mday, 0 as metric_book_investigations, 0 as metric_book_investigations_unique, 0 as metric_book_requests, 0 as metric_book_requests_unique, SUM(m.metric) as metric, 0 as metric_chapter_investigations_unique, 0 as metric_chapter_requests, 0 as metric_chapter_requests_unique, 0 as metric_title_investigations_unique, 0 as metric_title_requests_unique
                    FROM metrics m
                    INNER JOIN submission_file_settings sfs ON (m.assoc_type IN (515, 531) AND m.assoc_id = sfs.submission_file_id AND sfs.setting_name = 'chapterId')
                    WHERE m.assoc_type IN (515, 531) AND m.metric_type = 'omp::counter' AND (m.country_id IS NOT NULL OR m.region IS NOT NULL OR m.city IS NOT NULL)
                    GROUP BY m.load_id, m.context_id, m.submission_id, m.country_id, m.region, m.city, mday) AS t
            ";
        if (substr(Config::getVar('database', 'driver'), 0, strlen('postgres')) === 'postgres') {
            $metricChapterInvestigationsUpsertSql .= '
        ...
        DB::statement($metricChapterInvestigationsUpsertSql);

        // metric_chapter_requests
        // means: galley files that belong to a chapter
        // chapter landing page did not exist yet
        $metricChapterRequestsUpsertSql = "
                INSERT INTO metrics_counter_submission_geo_daily (load_id, context_id, submission_id, country, region, city, date, metric_book_investigations, metric_book_investigations_unique, metric_book_requests, metric_book_requests_unique, metric_chapter_investigations, metric_chapter_investigations_unique, metric_chapter_requests, metric_chapter_requests_unique, metric_title_investigations_unique, metric_title_requests_unique)
                SELECT * FROM (SELECT m.load_id, m.context_id, m.submission_id, COALESCE(m.country_id, ''), COALESCE(m.region, ''), COALESCE(m.city, ''), {$dayFormatSql} as mday, 0 as metric_book_investigations, 0 as metric_book_investigations_unique, 0 as metric_book_requests, 0 as metric_book_requests_unique, 0 as metric_chapter_investigations, 0 as metric_chapter_investigations_unique, SUM(m.metric) as metric, 0 as metric_chapter_requests_unique, 0 as metric_title_investigations_unique, 0 as metric_title_requests_unique
                    FROM metrics m
                    INNER JOIN submission_file_settings sfs ON (m.assoc_type = 515 AND m.assoc_id = sfs.submission_file_id AND sfs.setting_name = 'chapterId')
                    WHERE m.assoc_type = 515 AND m.metric_type = 'omp::counter' AND (m.country_id IS NOT NULL OR m.region IS NOT NULL OR m.city IS NOT NULL)
                    GROUP BY m.load_id, m.context_id, m.submission_id, m.country_id, m.region, m.city, mday) AS t
            ";
        if (substr(Config::getVar('database', 'driver'), 0, strlen('postgres')) === 'postgres') {
            $metricChapterRequestsUpsertSql .= '
        ...
        DB::statement($metricChapterRequestsUpsertSql);
        ...
        // Migrate to metrics_counter_submission_geo_monthly table
        $monthFormatSql = "DATE_FORMAT(STR_TO_DATE(gd.date, '%Y-%m-%d'), '%Y%m')";
        if (substr(Config::getVar('database', 'driver'), 0, strlen('postgres')) === 'postgres') {
            $monthFormatSql = "to_char(gd.date, 'YYYYMM')";
        }
        DB::statement("
            INSERT INTO metrics_counter_submission_geo_monthly (context_id, submission_id, country, region, city, month, metric_book_investigations, metric_book_investigations_unique, metric_book_requests, metric_book_requests_unique, metric_chapter_investigations, metric_chapter_investigations_unique, metric_chapter_requests, metric_chapter_requests_unique, metric_title_investigations_unique, metric_title_requests_unique)
            SELECT gd.context_id, gd.submission_id, gd.country, gd.region, gd.city, {$monthFormatSql} as month, SUM(gd.metric_book_investigations), SUM(gd.metric_book_investigations_unique), SUM(gd.metric_book_requests), SUM(gd.metric_book_requests_unique), SUM(gd.metric_chapter_investigations), SUM(gd.metric_chapter_investigations_unique), SUM(gd.metric_chapter_requests), SUM(gd.metric_chapter_requests_unique), SUM(gd.metric_title_investigations_unique), SUM(gd.metric_title_requests_unique)
            FROM metrics_counter_submission_geo_daily gd
            GROUP BY gd.context_id, gd.submission_id, gd.country, gd.region, gd.city, month
        ");

        // Delete the entries with the metric type omp::counter from the DB table metrics -> they were migrated above
        if (Schema::hasTable('metrics')) {
            DB::statement("DELETE FROM metrics WHERE metric_type = 'omp::counter'");
            if (substr(Config::getVar('database', 'driver'), 0, strlen('postgres')) === 'postgres') {
