...
class I6782_Metrics extends Migration
{
    ...
    public function up(): void
    {
        ...
        DB::statement("
            INSERT INTO metrics_geo_daily (load_id, context_id, submission_id, country, region, city, date, metric, metric_unique)
            ...
            FROM metrics m
            WHERE m.assoc_type IN (515, 531, 1048585) AND m.metric_type = 'omp::counter' AND (m.country_id IS NOT NULL OR m.region IS NOT NULL OR m.city IS NOT NULL)
        ...
        Schema::table('metrics_submission_geo_daily', function (Blueprint $table) {
            ...
            if (!array_key_exists('msgd_tmp_index', $indexesFound)) {
                $table->index(['country', 'region'], 'msgd_tmp_index');
            }
        });
