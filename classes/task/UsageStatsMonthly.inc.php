...
class UsageStatsMonthly extends ScheduledTask
{
    ...
    public function executeActions(): bool
    {
        ...
        DB::statement(
			...
			SELECT gd.context_id, gd.submission_id, COALESCE(gd.country, ''), COALESCE(gd.region, ''), COALESCE(gd.city, ''), DATE_FORMAT(gd.date, '%Y%m') as month, SUM(gd.metric_book_investigations), SUM(gd.metric_book_investigations_unique), SUM(gd.metric_book_requests), SUM(gd.metric_book_requests_unique), SUM(gd.metric_chapter_investigations), SUM(gd.metric_chapter_investigations_unique), SUM(gd.metric_chapter_requests), SUM(gd.metric_chapter_requests_unique), SUM(gd.metric_title_investigations_unique), SUM(gd.metric_title_requests_unique) FROM metrics_counter_submission_geo_daily gd WHERE month <> ? GROUP BY gd.context_id, gd.submission_id, gd.country, gd.region, gd.city, month
			",
            [$currentMonth]
        );
