...
class VGWortPlugin extends GenericPlugin {
	...
	function register($category, $path) {
		...
		if ($success && $this->getEnabled()) {
			...
			HookRegistry::register('Templates::Controllers::Modals::SubmissionMetadata::CatalogEntryTabs::AdditionalItems', array($this, 'vgWortEntryTab'));
	...
	function handleTemplateInclude($hookName, $params) {
		$smarty =& $params[0];
		$template =& $params[1];

		switch ($template['smarty_include_tpl_file']) {
			case 'catalog/book/bookInfo.tpl':
				HookRegistry::register ('TemplateManager::include', array(&$this, 'handleTemplateInclude'));
				break;
			case 'catalog/book/bookFiles.tpl':
