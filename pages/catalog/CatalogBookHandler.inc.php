...
class CatalogBookHandler extends Handler
{
    ...
    public function book($args, $request)
    {
        ...
        if (!HookRegistry::call('CatalogBookHandler::book', [&$request, &$submission])) {
            ...
            if ($submission->getStatus() == Submission::STATUS_PUBLISHED && !$request->isDNTSet()) {
                if ($this->isChapterRequest) {
                    event(new UsageEvent(Application::ASSOC_TYPE_CHAPTER, $this->chapter->getId(), $submission->getContextId(), $submission->getId(), null, null, $this->chapter->getId()));
                } else {
                    event(new UsageEvent(Application::ASSOC_TYPE_SUBMISSION, $submission->getId(), $submission->getContextId(), $submission->getId()));
                }
            }
            return;
        }
    ...
    public function download($args, $request, $view = false)
    {
        ...
        if ($submissionFile->getDirectSalesPrice() === '0' || ($user && $ompCompletedPaymentDao->hasPaidPurchaseFile($user->getId(), $submissionFile->getId()))) {
            ...
            if ($submissionFile->getData('assocId') == $publicationFormat->getId() && !$request->isDNTSet()) {
                ...
                event(new UsageEvent($assocType, $submissionFile->getId(), $submission->getContextId(), $submission->getId(), $publicationFormat->getId(), $submissionFile->getData('mimetype'), $chapterId));
            }
            $returner = true;
